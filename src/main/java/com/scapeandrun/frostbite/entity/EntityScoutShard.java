    ...
    public void onLivingUpdate() {
        ...
        if (kind() == 8 || kind() == 9) {
            ...
            if (wall != null || hit || to.distanceTo(locked) < 1.7 || tick >= strike + 42) {
                ...
                com.scapeandrun.frostbite.network.PacketScoutImpact.send(this, point, 0);
                setDead();
            } else {
                ...
                for (int i = 0; i < 3; i++)
                    world.spawnEntity(
                            new EntityScoutShard(
                                    ...
                                    1,
                                    slot * 3 + i));
                com.scapeandrun.frostbite.network.PacketScoutImpact.send(this, hit, 0);
                setDead();
            } else {
                setPosition(to.x, to.y, to.z);
                motionX = motionY = motionZ = 0;
