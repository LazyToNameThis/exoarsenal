...
public abstract class EntityFrigidRobot extends EntityMob implements IAnimatable {
    ...
    protected void dropFewItems(boolean hit, int looting) {
        if (kind() == Kind.SHIELDER && rand.nextFloat() < .2F)
            dropItem(com.scapeandrun.frostbite.registry.ModContent.SUSPICIOUS_CONTROLLER, 1);
        int metal = kind() == Kind.ARTHROPOD ? 3 + rand.nextInt(3) : rand.nextInt(2);
        if (metal > 0) dropItem(com.scapeandrun.frostbite.registry.ModContent.FRIGID_METAL, metal);
    }
    ...
    public boolean getCanSpawnHere() {
        return com.scapeandrun.frostbite.world.FrigidSpawnBiomes.isCold(
                        world.getBiome(getPosition()))
