...
public final class ScoutBossBar {
    ...
    public static void render(RenderGameOverlayEvent.BossInfo event) {
        ...
        float scale =
                Math.min(
                        1F,
                        (event.getResolution().getScaledWidth() - 12F) / OrnateBossBarLayout.WIDTH);
        int
                x =
                        (event.getResolution().getScaledWidth()
                                        - Math.round(OrnateBossBarLayout.WIDTH * scale))
                                / 2,
                y = Math.max(2, event.getY() - 8);
        event.setCanceled(true);
        event.setIncrement((int) Math.ceil(OrnateBossBarLayout.HEIGHT * scale) + 4);
        ...
        else if (excavator) OrnateBossBarLayout.excavator(canvas, a, trail.value[0], tick);
        else if (eyes)
            OrnateBossBarLayout.eyes(
