    ...
    public static void draw(RenderGameOverlayEvent.Post e) {
        ...
        for (EntityExcavator boss :
            ...
            if (boss.introTick() == -1 && boss.parryCount() > 0) {
                if (com.scapeandrun.frostbite.entity.EntityDraedon.trialActor(boss)) continue;
                float alpha =
                        boss.expertStage() == 6
                                ? Math.min(1, (40 - boss.attackTick()) / 12F)
                                : Math.min(1, boss.attackTick() / 10F);
                EncounterTitle.draw(
                        com.scapeandrun.frostbite.entity.SeerObserverIntro.TITLE,
                        com.scapeandrun.frostbite.entity.SeerObserverIntro.NAME,
