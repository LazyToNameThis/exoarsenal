...
public final class WeaponCombatClient {
    ...
    public static void click(MouseEvent e) {
        ...
        if (m.player == null || m.currentScreen != null || e.getButton() != 0 || !e.isButtonstate())
            return;
        if (com.scapeandrun.frostbite.event.RmorEventHandler.isDefenseForm(m.player)) return;
                ...
                if (!m.player.isInvisible()) {
                    int type =
                            e.getItemStack().getItem()
                                            instanceof com.scapeandrun.frostbite.item.ItemEnergyGun
                                    ? ((com.scapeandrun.frostbite.item.ItemEnergyGun)
                                                    e.getItemStack().getItem())
                                            .getType()
                                            .ordinal()
                                    : -1;
