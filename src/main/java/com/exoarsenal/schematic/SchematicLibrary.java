package com.exoarsenal.schematic;
...
public final class SchematicLibrary {
    ...
    public static File folder(MinecraftServer server) {
        File folder = server.getFile("config/" + ExoArsenal.MODID + "/schematics");
        if (!folder.isDirectory() && !folder.mkdirs()) {
            ExoArsenal.LOGGER.warn("Could not create schematic library at {}", folder);
        }
        writeReadme(folder);
        return folder;
    }
    ...
    public static Data load(MinecraftServer server, String fileName) {
        ...
        try {
            ...
            ExoArsenal.LOGGER.warn("Could not resolve schematic {}", fileName, error);
            return null;
        }
    }
    ...
    private static void writeReadme(File folder) {
        ...
        try (FileWriter writer = new FileWriter(readme)) {
            ...
            ExoArsenal.LOGGER.warn("Could not write schematic library README", error);
        }
    }
}
