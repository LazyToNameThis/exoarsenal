...
public class EntityDesertScourge extends EntityMob implements IAnimatable {
    ...
    public static boolean desert(World w, BlockPos p) {
        ...
        return BiomeDictionary.hasType(b, BiomeDictionary.Type.SANDY)
                && !com.exoarsenal.world.FrigidSpawnBiomes.isCold(b);
    }
    ...
    public void onLivingUpdate() {
        ...
        if (!balanced) {
            ...
            if (!nuisance()) dataManager.set(EXPERT, ExoArsenalWorldSettings.get(world).isExpert());
    ...
    protected void dropFewItems(boolean hit, int looting) {
        if (nuisance()) return;
        if (expert()) dropItem(com.exoarsenal.registry.ModContent.DESERT_SCOURGE_BAG, 1);
        else
            for (ItemStack drop : com.exoarsenal.expedition.ScourgeLoot.roll(rand, false))
                entityDropItem(drop, 0);
        if (rand.nextInt(10) == 0)
            entityDropItem(new ItemStack(com.exoarsenal.expedition.ExpeditionContent.TROPHY), 0);

        entityDropItem(new ItemStack(com.exoarsenal.expedition.ExpeditionContent.RELIC), 0);
        if (rand.nextInt(100) == 0)
            dropItem(com.exoarsenal.expedition.ExpeditionContent.THANK_YOU, 1);
        if (com.exoarsenal.expedition.ScourgeLoot.Progress.get(world).firstKill())
            dropItem(com.exoarsenal.expedition.ExpeditionContent.LORE, 1);
    }
