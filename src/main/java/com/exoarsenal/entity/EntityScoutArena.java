...
public final class EntityScoutArena extends Entity {
    ...
    public void raise() {
        ...
        for (EntityPlayer p :
            ...
            p.getEntityData().setTag("ExoArsenalArenaReturn", journal);
    ...
    public void finish() {
        if (!world.isRemote && raised())
            for (EntityPlayer p : world.playerEntities) {
                ...
                p.fallDistance = 0;
                p.getEntityData().removeTag("ExoArsenalArenaReturn");
            }
        setDead();
    }
    ...
    public static void rejoin(
        ...
        if (p.world.isRemote || !p.getEntityData().hasKey("ExoArsenalArenaReturn")) return;
        NBTTagCompound n = p.getEntityData().getCompoundTag("ExoArsenalArenaReturn");
