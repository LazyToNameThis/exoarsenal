...
import com.exoarsenal.world.ExcavatorTerrain;
...
import java.util.*;
import static com.exoarsenal.entity.ExcavatorClassicScore.*;
...
public final class EntityExcavator extends EntityMob {
    ...
    void parryLanding(EntityPlayer player, Vec3d at, Vec3d direction) {
        com.exoarsenal.network.PacketExcavatorParry.send(this, player, at, direction, 1);
        dust(at, 42);
        world.playSound(
                null,
                at.x,
    ...
    private void tickFinale() {
        ...
        if (finale() == 1) {
            ...
            if (t % 16 == 1)
                com.exoarsenal.network.PacketExcavatorParry.send(
    ...
    protected void dropFewItems(boolean hit, int looting) {
        if (dataManager.get(EXPERT)) {
            dropItem(com.exoarsenal.registry.MachineRewards.EXCAVATOR_BAG, 1);
            return;
        }
        ExcavatorLoot loot = ExcavatorLoot.roll(rand);
        dropItem(com.exoarsenal.expedition.ExpeditionContent.SCRAP, loot.scrap);
        dropItem(com.exoarsenal.expedition.ExpeditionContent.CORE, loot.cores);
        dropItem(com.exoarsenal.registry.MachineRewards.PROBE, loot.probes);
    }
