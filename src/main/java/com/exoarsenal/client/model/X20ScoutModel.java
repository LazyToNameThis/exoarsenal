...
public final class X20ScoutModel extends AnimatedGeoModel<EntityX20Scout> {
    ...
    public ResourceLocation getModelLocation(EntityX20Scout object) {
        return new ResourceLocation(ExoArsenal.MODID, "geo/x20_scout_humanoid.geo.json");
    }

    @Override
    public ResourceLocation getTextureLocation(EntityX20Scout object) {
        return com.exoarsenal.client.FrigidTexture.location();
    }

    @Override
    public ResourceLocation getAnimationFileLocation(EntityX20Scout object) {
        return new ResourceLocation(
                ExoArsenal.MODID, "animations/x20_scout_humanoid.animation.json");
    }
    ...
    public void setLivingAnimations(EntityX20Scout scout, Integer id, AnimationEvent event) {
        ...
        if (scout.getAttack() != com.exoarsenal.entity.ScoutCombatPattern.MORPH) {
        ...
        float accretion =
                scout.getAttack() == com.exoarsenal.entity.ScoutCombatPattern.HAMMER_RICOCHET
            ...
            double yaw =
                    scout.prevRenderYawOffset
                            + net.minecraft.util.math.MathHelper.wrapDegrees(
                                    ...
                                    * partial;
            double[] position =
                    com.exoarsenal.entity.ScoutSpace.boneTranslation(
        ...
        visible(
                "leftSwordCrestIce",
                scout.isHardpointAlive(com.exoarsenal.entity.EntityScoutHardpoint.MINIGUN_LEFT));
        visible("rightSwordCrestIce", false);
        visible(
                "leftShoulderIce",
                scout.isHardpointAlive(com.exoarsenal.entity.EntityScoutHardpoint.LASER_LEFT));
        visible(
                "rightShoulderIce",
                scout.isHardpointAlive(com.exoarsenal.entity.EntityScoutHardpoint.LASER_RIGHT));
        visible("clawArm", scout.isHardpointAlive(com.exoarsenal.entity.EntityScoutHardpoint.CLAW));
        visible("viceArm", scout.isHardpointAlive(com.exoarsenal.entity.EntityScoutHardpoint.VICE));
        ...
        float spread = com.exoarsenal.client.ScoutHandPose.scissorSpread(scout.getAttack(), tick);
        ...
        float extension =
                form == 2
                        ? com.exoarsenal.client.ScoutHandPose.scissorExtension(
        ...
        for (String n : new String[] {"leftWingIce", "rightWingIce"}) {
            ...
            boolean folded =
                    scene == 0
                            && scout.getAttack()
                                    == com.exoarsenal.entity.ScoutCombatPattern.EXPERT_BOXING
                            && tick < 196;
            float fold =
                    folded
                            ? com.exoarsenal.client.ScoutBrawlPose.curve(
        ...
        boolean rangeReel =
                scout.getAttack() == com.exoarsenal.entity.ScoutCombatPattern.RANGE_REEL;
        ...
        for (boolean right : new boolean[] {false, true}) {
            double deployedVice =
                    com.exoarsenal.entity.ScoutViceMotion.extension(scout.getAttack(), tick, right);
            ...
            if (scene == 0 && deployedVice > .001) {
                ...
                float open =
                        .12F
                                + (float)
                                                (1
                                                        - com.exoarsenal.entity.ScoutViceMotion
                                                                .closure(
                                                                        scout.getAttack(),
                                                                        tick,
                                                                        right))
        ...
        java.util.Map<String, float[]> poses =
                com.exoarsenal.client.ScoutBrawlPose.sample(
                        scene,
                        scout.getAttack(),
                        poseTick,
                        scout.limbSwing,
        ...
        if (scene == 0 && scout.isExpert() && scout.getPhase() == EntityX20Scout.PHASE_DUO)
            com.exoarsenal.client.ScoutDualWeaponPose.apply(poses, scout.getAttack(), poseTick);
        ...
        if (drawing) {
            float t = poseTick;
            boolean airborne = com.exoarsenal.client.ScoutEntrance.airborne(t);
            visible("leftSword", !airborne);
            visible("leftBladeIce", !airborne);
            visible("thrownSword", airborne);
            visible("thrownSwordIce", airborne);
            if (airborne) {
                double[] p = com.exoarsenal.client.ScoutEntrance.sword(t);
                double[] local =
                        com.exoarsenal.entity.ScoutSpace.boneTranslation(0, p[0], p[1], p[2]);
            ...
            getBone("leftSword").setRotationX(com.exoarsenal.client.ScoutEntrance.spin(t));
            float cape = com.exoarsenal.client.ScoutEntrance.cape(t);
    ...
    private void animateDefeat(float tick) {
        com.exoarsenal.client.ScoutDefeatPose.apply(tick, this::getBone);
    }
        ...
        double yaw =
                scout.prevRenderYawOffset
                        + net.minecraft.util.math.MathHelper.wrapDegrees(
                                        scout.renderYawOffset - scout.prevRenderYawOffset)
                                * partial;
        double[] p = com.exoarsenal.entity.ScoutSpace.boneTranslation(yaw, d.x, d.y, d.z);
    ...
    private void animateHands(EntityX20Scout scout, float partial) {
        ...
        getBone("leftWrist")
                .setRotationX(
                        (float)
                                Math.toRadians(
                                        com.exoarsenal.client.ScoutHandPose.wrist(
                                                scout.getAttack(), tick)));
        for (int i = 0; i < digits.length; i++)
            for (boolean right : new boolean[] {false, true})
                for (boolean tip : new boolean[] {false, true}) {
                    float[] pose =
                            com.exoarsenal.client.ScoutHandPose.digit(
