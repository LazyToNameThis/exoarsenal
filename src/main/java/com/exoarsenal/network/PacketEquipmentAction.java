    ...
    public static class Handler implements IMessageHandler<PacketEquipmentAction, IMessage> {
        ...
        private static void handle(EntityPlayerMP player, int action) {
            if (action == 40 || action == 41) {
                com.exoarsenal.expedition.WulfrumCombat.request(player, action - 40);
                return;
            }
            if (action == 8) {
                com.exoarsenal.expedition.ExpeditionEvents.fire(player);
                return;
            }
            if (action == TOGGLE_DEFENSE && com.exoarsenal.expedition.WulfrumArmor.full(player)) {
                com.exoarsenal.expedition.ExpeditionEvents.toggle(player);
                return;
            }
            if (com.exoarsenal.combat.WeaponCombat.busy(player) && action != RTOOL_STOP) return;
