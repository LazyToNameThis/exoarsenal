            ...
            player.getServerWorld()
                    .addScheduledTask(
                            () -> {
                                ...
                                boolean allowed = owner || player.canUseCommand(2, "exoexpert");
                                ...
                                ExoArsenalWorldSettings settings =
                                        ExoArsenalWorldSettings.get(player.world);
                                if (message.action > 0
                                        && allowed
                                        && !fighting
        ...
        public static final class Handler implements IMessageHandler<State, IMessage> {
            public IMessage onMessage(State message, MessageContext context) {
                net.minecraft.client.Minecraft.getMinecraft()
                        .addScheduledTask(
                                () ->
                                        com.exoarsenal.client.DifficultySelector.receive(
                                                message.difficulty,
                                                message.allowed,
                                                message.fighting));
                return null;
